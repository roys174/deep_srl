...
if __name__ == "__main__":
  ...
  parser.add_argument('-c', '--case_sensitive',
                      action='store_true',
                      help='Keep words in original case')
  parser.add_argument('-p', '--prevent_new_words',
                      action='store_true',
                      help="Don't allow new words in prediction")
  parser.add_argument('-k', '--keep_embeddings',
                      action='store_true',
                      help="Keep model embeddings")


  args = parser.parse_args()
  config = configuration.get_config(os.path.join(args.model, 'config'))

  config.embedding_file = args.embeddings
  ...
  for i in range(num_ensemble_models):
    ...
    scores, data, test_sentences, test_data = get_scores(config,
                                                         ...
                                                         not args.prevent_new_words,
                                                         not args.keep_embeddings)
