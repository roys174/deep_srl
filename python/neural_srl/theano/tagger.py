...
class BiLSTMTaggerModel(object):
    ...
    def load(self, filepath, word_dict, replace_vocab=True):
        """ Load model parameters from file.
        """
        all_params = numpy.load(filepath)
        for param in self.params:
            if param.name in all_params:
                print param.name,"found!"
                ...
                if param.name.startswith('embedding') and self.embedding_shapes[0][0] > vals.shape[0]:
                    ...
                    print "embedding size:",self.embedding_shapes[0][0], vals.shape, param.get_value().shape, new_vals.shape
                    if param.name.startswith('embedding_0'):
                        if replace_vocab:
                            # for i in range(10):
                            #     w = word_dict.idx2str[i]
                            #     print(w, param.get_value()[i][:10])
                            print "Using new embeddings for in-vocab words"
                            # for i in range(5):
                            #     print i,":", vals[i][:5], param.get_value()[i][:5]

                            for i in [START_MARKER, END_MARKER, UNKNOWN_TOKEN]:
                                old_value = None
                                if i in word_dict.str2idx:
                                    if old_value is None:
                                        old_value = param.get_value()

                                    index = word_dict.str2idx[i]
                                    # print(index,old_value is None)
                                    # print("Setting",i,"from",old_value[index][:10],"to be",vals[index][:10])
                                    old_value[index] = vals[index]


                            if old_value is not None:
                                param.set_value(old_value)
                                # print("maybe now? Setting", i, "from", param.get_value()[index][:10], "to be", vals[index][:10])

                            for i in range(10):
                                w = word_dict.idx2str[i]
                                print(w, param.get_value()[i][:10])
                        else:
                            print "Using original embeddings for in-vocab words"
