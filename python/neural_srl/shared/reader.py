...
def get_embeddings(word, embeddings):
  return embeddings[word] if word in embeddings else embeddings[UNKNOWN_TOKEN]
...
def get_postag_test_data(filepath, config, word_dict, label_dict, lower_case=True, allow_new_words=True):
